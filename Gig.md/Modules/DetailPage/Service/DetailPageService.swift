...
class DetailPageService {
    ...
    func applyToJob(JobId: Int, completion: @escaping(Result <JobDetailModel, Error>) -> Void) {
        DispatchQueue.main.async {
            self.networkService.put(url: APIConstants.url,
                                    path:  "/jobs/\(JobId)",
                                    body: nil,
                                    httpHeaders: ["authorization": CommonVars.token]) { result in
                switch result {
                case .success(let data):
                    print(data)
                    if let decodedData = try? self.decoder.decode(JobDetailModel.self, from: data) {
